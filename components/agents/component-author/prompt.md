...
Place every component flat under the top-level `components/<kind>s/<slug>/` tree (for example `components/skills/<slug>/`, `components/agents/<slug>/`, `components/workflows/<slug>/`). There are no per-bucket subtrees such as `components/harness/`.

Record which domain a component belongs to with the single `domain` field in its manifest: `domain: harness` for HarnessKit maintenance capabilities (component authoring, adapter authoring, validation, repository-specific harness operations), `domain: scm` or `domain: work` for those toolchains, and `domain: core` for reusable engineering components meant to be installed into target AI tools later.
