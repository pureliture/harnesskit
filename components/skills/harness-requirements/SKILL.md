...
Before asking, inspect the context the user already provided:

- use supplied reference packets, local paths, existing docs, ADRs, glossary/domain vocabulary, issue bodies, and conversation history when available;
- if a missing answer is discoverable from those sources, record it as an inference with source context rather than asking;
- if the user explicitly requests a grilling-style intake, still ask one decision at a time and include a recommended option plus alternatives;
- if the user asks for PRD-like output, translate that into HarnessKit requirements sections and keep issue publication out of scope.
